...
export default async function PreviewPage({ params, searchParams }: PreviewPageProps) {
  ...
  // Tenant resolution for preview (service role, bypasses RLS):
  // 1. If ?platform= query param is set, look up by ID
  // 2. Otherwise, look up by host (works for freshly provisioned
  //    private platforms that have preview_domain set)
  ...
  return (
    <div className="flex flex-col min-h-screen bg-surface">
      ...
      <div role="alert" className="sticky top-0 z-50 bg-amber-500 text-black">
        <div className="max-w-[1280px] mx-auto px-4 py-2 flex items-center gap-3 text-sm font-medium">
          ...
          <span className="text-xs opacity-75 hidden sm:inline">Preview link verloopt na 1 uur</span>
        </div>
      </div>

      <TenantHeader tenant={tenant} showSearch={false} />

      <Wegwijzer back={{ label: 'Overzicht', href: '/' }} items={wegwijzerItems} />
      ...
      <ApplyButton jobUrl={job.url} jobId={job.id} jobTitle={job.title} isExpired={!!isExpired} />
    </div>
  )
}
