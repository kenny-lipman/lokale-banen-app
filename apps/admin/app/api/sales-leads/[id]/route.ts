...
async function getHandler(_req: NextRequest, _auth: AuthResult, ctx: RouteContext) {
  ...
  const { data, error } = await supabase
    ...
    .select(
      'id,status,input_url,input_domain,owner_config_id,scrape_vacancies,manual_vacancies,enrichments,master_record,selected_contacts,pipedrive_org_id,pipedrive_deal_id,existing_pipedrive_org_id,error,created_at,updated_at',
    )
    .eq('id', id)
    .maybeSingle()
