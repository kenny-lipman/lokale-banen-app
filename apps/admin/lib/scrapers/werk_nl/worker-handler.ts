/**
 * Gedeelde handler voor de werk.nl detail-verrijking worker (Fase 2/3).
 *
 * Geclaimd via werk_nl_scrape_queue (atomic via werknl_claim_batch RPC,
 * FOR UPDATE SKIP LOCKED). Daardoor kunnen meerdere worker-routes parallel
 * draaien zonder dubbel werk: elke instance claimt disjuncte rijen.
 *
 * Gebruikt door /api/scrapers/werk-nl/worker en .../worker-2 (twee cron-instances).
 */
...
export async function runWorkerHandler(req: NextRequest): Promise<NextResponse> {
